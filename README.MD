...
```
Client ──> HTTP Router (chi) ──> Middleware (Auth, RBAC, Logger)
                              │
                    ┌─────────┴─────────┐
                    ▼                   ▼
                  Handler        Service Layer (New)
               (Simple CRUD)    (Complex Business Logic)
                    │                   │
                    │           ┌───────┴───────┐
                    ▼           ▼               ▼
                  Repository Layer        Redis Queue ──> Worker Container
                        │                                  (Async Email)
                        ▼
                    Database (MySQL)
```
...
🐳 **QMenggunakan Docker (Rekomendasi)**
